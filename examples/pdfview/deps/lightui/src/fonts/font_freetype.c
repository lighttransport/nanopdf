 ...
 *
 * Compiled only when LUI_HAVE_FREETYPE is defined.
...
lui_glyph_entry_t *lui__ft_rasterize_and_cache(lui_font_t *f, uint16_t glyph_id)
{
    ...
    if (FT_Load_Glyph(face, glyph_id,
        ...
        uint8_t dummy = 0;
        return lui__cache_insert(&f->cache_ft, (uint32_t)glyph_id,
                                  &dummy, 0, 0, 0, 0, adv);
    }
    ...
    lui_glyph_entry_t *e = lui__cache_insert(&f->cache_ft, (uint32_t)glyph_id,
                                              pixels, bm_w, bm_h,
                                              slot->bitmap_left,
                                              slot->bitmap_top,
                                              adv);
