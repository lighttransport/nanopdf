...
// Shared WinAnsi / MacRoman code->Unicode tables (one definition across TUs via
// C++17 inline variables). Named distinctly from the per-backend static copies.
inline const uint16_t kWinAnsiToUnicode[256] = {
...
inline const uint16_t kMacRomanToUnicode[256] = {
    0x0000, 0x0001, 0x0002, 0x0003, 0x0004, 0x0005, 0x0006, 0x0007,
    0x0008, 0x0009, 0x000A, 0x000B, 0x000C, 0x000D, 0x000E, 0x000F,
    0x0010, 0x0011, 0x0012, 0x0013, 0x0014, 0x0015, 0x0016, 0x0017,
    0x0018, 0x0019, 0x001A, 0x001B, 0x001C, 0x001D, 0x001E, 0x001F,
...
// Convenience wrapper: full PDF code->Unicode mapping for a font, using the
// shared tables + glyph-name / Adobe-Japan1 helpers. Use this from any TU
// (text extraction, export, ...) for consistent behaviour with the backends.
inline uint32_t map_char_to_unicode(const BaseFont* font, uint32_t char_code) {
  return map_char_to_unicode_generic(char_code, font, kWinAnsiToUnicode,
                                     kMacRomanToUnicode, glyph_name_to_unicode,
                                     adobe_japan1_cid_to_unicode_regular);
}
