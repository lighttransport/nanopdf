/*
 * lightui/vg/surface.h — Pixel-buffer surface
 *
 ...
 * `lui_surface_create` / `lui_surface_resize` call a malloc-style allocator.
 * To run in environments without libc heap, either:
 *   (a) use `lui_surface_wrap()` on caller-owned memory and skip create/destroy;
 *   (b) pass allocator callbacks to `lui_surface_create_ex()`;
 *   (c) compile the module with `-DLUI_VG_MALLOC=... -DLUI_VG_FREE=...` macros
 ...
 * flavor is compiled only when `LUI_VG_NO_STDIO` is not defined (default on).
 *
 * SPDX-License-Identifier: Apache-2.0
 */
#ifndef LIGHTUI_SURFACE_H
#define LIGHTUI_SURFACE_H
...
/**
 * lui_surface_t — an owned or borrowed pixel buffer.
 *
...
/**
 * lui_surface_view_t — frozen POD snapshot of a surface's pixel storage.
 *
 * The long-term plan is to make `lui_surface_t` opaque; tight hot loops
 ...
 * Obtain via `lui_surface_as_view()`.
 */
typedef struct {
    uint32_t  *pixels;
    int        width;
...
typedef void *(*lui_vg_alloc_fn)(void *userdata, size_t size);

/** Deallocator paired with lui_vg_alloc_fn. Passing NULL `ptr` is a no-op. */
typedef void  (*lui_vg_free_fn)(void *userdata, void *ptr);
...
typedef size_t (*lui_vg_write_fn)(void *userdata, const void *data, size_t len);
 ...
 * (malloc/free, overridable at compile time via LUI_VG_MALLOC / LUI_VG_FREE).
...
/**
 * Same as lui_surface_create() but uses caller-supplied allocator callbacks.
 * The same free_cb/userdata must later be passed to lui_surface_destroy_ex().
...
/**
 * Destroy a surface allocated with lui_surface_create().
 * Passing NULL is a no-op.
 */
void lui_surface_destroy(lui_surface_t *surface);

/**
 * Destroy a surface allocated with lui_surface_create_ex(), using the
 ...
 */
static inline lui_surface_view_t lui_surface_as_view(const lui_surface_t *surface)
{
    lui_surface_view_t v;
    if (!surface) {
 ...
 * Requires the optional PNG backend (compile with LUI_VG_ENABLE_PNG).
 ...
 * Requires the optional JPEG backend (compile with LUI_VG_ENABLE_JPEG).
...
/* -------------------------------------------------------------------------
 * Writers — file path flavor (uses stdio; compiled when LUI_VG_NO_STDIO
 * is NOT defined)
 * ------------------------------------------------------------------------- */
 ...
 * Requires LUI_VG_ENABLE_PNG at compile time.
 * Returns 0 on success, -1 on error.
 */
 ...
 * Requires LUI_VG_ENABLE_JPEG at compile time.
 * Returns 0 on success, -1 on error.
 */
 ...
 * lui_surface_save_jpeg() directly to control it. Unknown extensions fall
 * back to PPM.
 * Returns 0 on success, -1 on error.
 */
