 ...
 */
static void draw_run(const lui_run_t *r, lui_canvas_t *canvas,
                     ...
                     int letter_spacing)
{
    ...
    if (r->is_image) {
        ...
        if (r->width == r->image->width && r->height == r->image->height) {
            lui_canvas_blit(canvas, rx, ry, r->image, NULL);
        } else {
            lui_canvas_draw_image(canvas, rx, ry, r->width, r->height,
                                  r->image, NULL, LUI_IMAGE_FILTER_BILINEAR);
        }
    } else {
        /* Draw background fill behind this word if requested. */
        if (LUI_COLOR_A(r->bg) > 0) {
            int ry_top = layout_y + line_top;
            lui_canvas_fill_rect(canvas, rx, ry_top, r->width, line_h, r->bg);
        }
        ...
        if (r->flags & LUI_TEXT_UNDERLINE) {
            int uy = ry + 2;
            lui_canvas_draw_line(canvas, rx, uy, rx + r->width, uy, r->fg, 1);
        }
        /* Strikethrough */
        if (r->flags & LUI_TEXT_STRIKETHROUGH) {
            int sy = layout_y + line_top + (line_h / 2);
            lui_canvas_draw_line(canvas, rx, sy, rx + r->width, sy, r->fg, 1);
        }
    }
}
                           ...
                           lui_frame_clock_t *clk)
{
    ...
    for (int li = 0; li < tl->line_count; li++) {
        ...
        lui_rect_t line_rect = lui_rect_make(x, y + line->y,
                                              tl->total_width > 0
                                                  ? tl->total_width
        ...
        if (dirty && !lui_dirty_test(dirty, &line_rect)) continue;
        ...
        for (int ri = line->run_start;
            ...
            lui_rect_t run_rect = lui_rect_make(x + run->x, y + line->y,
                                                 run->width, line->height);

            if (dirty && !lui_dirty_test(dirty, &run_rect)) continue;

            if (dirty && !dirty->all) {
                /* Draw clipped to each dirty rect that intersects this run. */
                lui_rect_t saved = canvas->_clip;
                for (int di = 0; di < dirty->count; di++) {
                    lui_rect_t clip = lui_rect_intersect(&run_rect,
                                                          &dirty->rects[di]);
                    if (lui_rect_is_empty(&clip)) continue;
                    /* Further intersect with existing canvas clip. */
                    clip = lui_rect_intersect(&clip, &saved);
                    if (lui_rect_is_empty(&clip)) continue;
                    lui_canvas_set_clip(canvas, &clip);
